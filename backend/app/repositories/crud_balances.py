...
def calculate_user_balances(db: Session) -> List[UserBalance]:
    """
    Berechnet die Kontostände (Balances) aller existierenden User.
    Gibt eine Liste von UserBalance Pydantic-Models zurück.
    """
    ...
    # Formatierung für die Response als Pydantic Models (Option B)
    return [
        UserBalance(
            email=u.email,
            name=u.name,
